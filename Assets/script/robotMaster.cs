...
public class robotMaster : MonoBehaviour
{
    ...
    int curnum = 0;
    int maxnum  = 6;
    Dictionary<int, Joint> connectedBdy;
    int cleaned = 0;
    void Start()
    {
        resetContract = false;
        ...
        connectedBdy = new Dictionary<int, Joint>();
        //internalCounter = internalDelay;
    }
    public void UpdateTargetPos(){
        transform.parent.GetComponent<ClusterControl>().reached=true;
    }
    ...
    void Update(){
        while(cleaned>0){
            foreach(int keys in connectedBdy.Keys.ToList()){
            if(connectedBdy[keys] == null){
                connectedBdy.Remove(keys);
                cleaned -=1;
            }
            ...
            if(other.collider.TryGetComponent(out robotMaster rb)){
                ...
                if(!connectedBdy.ContainsKey(otherID)){
                    if (curnum<maxnum){
                        ...
                        connectedBdy.Add(otherID, joint);
